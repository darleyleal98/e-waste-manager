...
fun EquipamentStatusField(
    ...
    TextField(
        value = registrationViewModel.equipamentStatus,
        onValueChange = {
            registrationViewModel.equipamentStatus = it
            isValid = it.isNotEmpty()
        },
        isError = !isValid || registrationViewModel.equipamentStatus.isEmpty(),
        label = {
            Text(stringResource(R.string.status_do_equipamento), color = Color.White)
        },
        supportingText = {
            if (!isValid) {
                Text(
