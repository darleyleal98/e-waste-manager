...
fun TelephoneNumberField(
    ...
    TextField(
        value = registrationViewModel.telephoneNumber,
        onValueChange = {
            registrationViewModel.telephoneNumber = it
            isValid = it.isNotEmpty()
        },
        isError = !isValid || registrationViewModel.userName.isEmpty(),
        label = {
            Text(stringResource(R.string.numero_de_telefone), color = Color.White)
        },
        supportingText = {
            if (!isValid) {
                Text(
