...
fun EquipamentTypeField(
    ...
    TextField(
        value = registrationViewModel.equipamentType,
        onValueChange = {
            registrationViewModel.equipamentType = it
            isValid = it.isNotEmpty()
        },
        isError = !isValid || registrationViewModel.userName.isEmpty(),
        label = {
            Text(stringResource(R.string.tipo_do_equipamento), color = Color.White)
        },
        supportingText = {
            if (!isValid) {
                Text(
