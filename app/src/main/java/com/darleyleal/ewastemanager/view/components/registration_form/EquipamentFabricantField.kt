...
fun EquipamentFabricantField(
    ...
    TextField(
        value = registrationViewModel.equipamentFabricant,
        onValueChange = {
            registrationViewModel.equipamentFabricant = it
            isValid = it.isNotEmpty()
        },
        isError = !isValid || registrationViewModel.userName.isEmpty(),
        label = {
            Text(stringResource(R.string.fabricante_do_equipamento), color = Color.White)
        },
        supportingText = {
            if (!isValid) {
                Text(
