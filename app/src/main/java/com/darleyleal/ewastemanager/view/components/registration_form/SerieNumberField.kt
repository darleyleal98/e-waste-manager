...
fun SerieNumberField(
    ...
    TextField(
        value = registrationViewModel.serieNumber,
        onValueChange = {
            registrationViewModel.serieNumber = it
            isValid = it.isNotEmpty()
        },
        isError = !isValid || registrationViewModel.userName.isEmpty(),
        label = {
            Text(stringResource(R.string.numero_de_serie), color = Color.White)
        },
        supportingText = {
            if (!isValid) {
                Text(
